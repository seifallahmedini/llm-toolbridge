    ...
    async def generate(self, prompt, tools=None, tool_results=None, **kwargs):
        ...
        if not tool_results:
            return LLMResponse(
                ...
                tool_calls=[
                    ToolCall(
                        tool_name="adder", arguments={"a": 1, "b": 2}, call_id="call1"
                    )
                ],
            )
        else:
...
def test_register_tools_bulk():
    ...
    tool2 = Tool(
        ...
        parameters={
            "a": ParameterDefinition(type="number", description="A"),
            "b": ParameterDefinition(type="number", description="B"),
        },
        function=lambda a, b: a * b,
    )
...
def test_execute_sync_with_provider_tool_error():
    ...
    resp = bridge.execute_sync("add 1 and 2", [tool])
    assert (
        resp.content == "Result is 3" or resp.content is None
    )  # The error is not surfaced in content
